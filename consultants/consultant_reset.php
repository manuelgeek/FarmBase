...
$consultant_reset = new CONSULTANT();

if($consultant_reset->is_logged_in()!="")
{
	$consultant_reset->redirect('index');
}
...
if(isset($_POST['btn-reset']))
{
	...
	$stmt = $consultant_reset->runQuery("SELECT * FROM tbl_farmers WHERE email=:email_id AND phone='$phone'");
	...
	if($stmt->rowCount() == 0){
		...
		if($consultant_reset->reset_pass($email,$phone,$upass)){
			$msg3 = "<div class='alert alert-success col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2' >
					<button class='close ' data-dismiss='alert'>&times;</button>
					<span class='glyphicon glyphicon-info-sign'></span> &nbsp; Password Successfully changed. <a href='index' >Log In to account </a>
				</div>";
		} else {
			$msg3 = "<div class='alert alert-danger col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2' data-dismiss='alert'>
					<button class='close ' data-dismiss='alert'>&times;</button>
		...
		<div class="container">
			<div class="col-md-12">
				<div class="col-md-8 col-md-offset-2 "><br><br><br><br>
					<h2 class="h2 text-center">Consultant <small>Reset Password</small></h2><br><br><br><br>
						<?php
							if(isset($msg3)){
								echo $msg3;
							}
